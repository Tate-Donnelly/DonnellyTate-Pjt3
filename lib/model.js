...
class Model {
    ...
    name="";
    isCar=false;
    constructor(name,gl,objPath, mtlPath,position=mat4(),rotation=mat4()) {
        this.gl=gl;
        this.name=name;
        this.position=position;
        this.rotation=rotation;
        this.startingMatrix=mult(this.rotation, this.position);
        this.modelMatrix=this.startingMatrix;
        //this.dummyTexture();
    ...
    render(){
        ...
        for(let i = 0; i < m.length; i++){
            drawByMaterial(this,index,m[i]);
            if(this.canMakeShadow) makeShadows(m[i]);
            index++;
        }
    }
    ...
    calculateLen(){
        ...
        for(let i=0;i<this.faces.length;i++) {
            result=result+this.faces[i].faceVertices.length;
        }
        return result;
    }
    ...
    getModelMatrix(animate){
        if(animate && this.isCar) this.animateMatrix=mult(rotateY(1),this.animateMatrix);
    ...
    addChild(child) {
        ...
        child.parent=this;
        //console.log(this.name +" is "+child.name+"'s parent");
    }
