...
class Model {
    ...
    constructor(objPath, mtlPath,position=vec3(0.0,0.0,0.0),rotation=0) {
        this.gl=gl;
    ...
    renderModel(face){
        ...
        gl.uniformMatrix4fv(transMatrixLoc, false, flatten(translate(this.position)));
        gl.uniformMatrix4fv(rotationMatrixLoc, false, flatten(rotateY(this.rotation)));

        gl.bindBuffer(gl.ARRAY_BUFFER, null);
        gl.drawArrays(gl.TRIANGLES,0,face.faceVertices.length);
    }

    setCurrentMaterial(face){
        face.texture = gl.createTexture();
    ...
    dummyTexture() {
        ...
        // Set behavior for s-t texture coordinates outside the 0.0-1.0 range
        let color = vec4(Math.floor(Math.random() * 255), Math.floor(Math.random() * 255), Math.floor(Math.random() * 255), 255);

        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 2, 2, 0, gl.RGBA, gl.UNSIGNED_BYTE,
            new Uint8Array([color[0], color[1], color[2], color[3], color[0], color[1], color[2], color[3], color[0], color[1], color[2], color[3], color[0], color[1], color[2], color[3]]));
    ...
    parseMtlFile(mtlFile) {
        ...
        for (let currLine = 0; currLine < mtlLines.length; currLine++) {
            ...
            else if (line.startsWith("Ka")) {       // Material diffuse definition
                let values = line.match(/[+-]?([0-9]+[.])?[0-9]+/g);
                this.ambientMap.set(currMaterial, [parseFloat(values[0]), parseFloat(values[1]), parseFloat(values[2]), 1.0]);
            }
